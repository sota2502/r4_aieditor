...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { action_state_id: @target.action_state_id, value: @target.value } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { action_state_id: @target.action_state_id, value: @target.value } }
